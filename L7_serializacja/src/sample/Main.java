...
public class Main extends Application implements Serializable, Initializable
{
    ...
    toSerialize serializeTo = new toSerialize();
    ...
    public void start(Stage stage)
    {
        ...
        MenuItem menuReadFile = new MenuItem("Open file (serializable)");
        MenuItem menuImportFile = new MenuItem("Import file");
        MenuItem menuExportFile = new MenuItem("Export file");

        menuFile.getItems().addAll(menuSaveFile, menuReadFile, menuImportFile, menuExportFile, close);
        //dodanie close do "File"
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //lista list, bo normalnie problem z odczytem
        List<String> choices = new ArrayList<>();
        choices.add("DO ZROBIENIA");
        choices.add("W TRAKCIE");
        choices.add("ZROBIONE");
        ...
        menuSaveFile.setOnAction((ActionEvent t) ->
        {
            //okienko wyboru listy
            ChoiceDialog<String> dialog = new ChoiceDialog<>("TO DO", choices);
            dialog.setTitle("Choice List");
            dialog.setHeaderText("Choise list to save");

            Optional<String> result = dialog.showAndWait();
            if (result.isPresent()) //jesli cos wybrano
            {
                //jedyny sluszy format xD
                ...
                if(file!=null)
                {
                    //observable list na array list w zaleznosci od wyboru
                    ArrayList<Item> list1Array = new ArrayList<>();
                    switch (result.get())
                    {
                        case "DO ZROBIENIA":
                            list1Array = new ArrayList<>(listToDo.getItems());
                            break;
                        case "W TRAKCIE":
                            list1Array = new ArrayList<>(listIn.getItems());
                            break;
                        case "ZROBIONE":
                            list1Array = new ArrayList<>(listDone.getItems());
                            break;
                    }
                    //ArrayList<Item> list1Array = new ArrayList<>(list1.getItems());
...
//            serializeTo.castToDo = new ArrayList<>(itemsToDo);
//            serializeTo.castIn = new ArrayList<>(itemsIn);
//            serializeTo.castDone = new ArrayList<>(itemsDone);
//            try(ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream("saveFile.bin"))){
//                outputStream.writeObject(serializeTo);//zapis do strumienia
//                outputStream.close();
...
//            }
        ...
        fileChooserSave.setTitle("Load");

        //odczyt z pliku
        menuReadFile.setOnAction((ActionEvent t) -> {
            fileChooserLoad.getExtensionFilters().addAll(
                    new FileChooser.ExtensionFilter("saveFile", "*.bin")
            );

            //pokazanie okna
            File file = fileChooserLoad.showOpenDialog(stage);

            if(file != null)
            {
                //observable list na array list
                ArrayList<Item> list1Array = new ArrayList<>();
                ...
                try
                {
                    ...
                    list1Array = (ArrayList<Item>) ois.readObject();

                    ChoiceDialog<String> dialog = new ChoiceDialog<>("TO DO", choices);
                    dialog.setTitle("Choice List");
                    dialog.setHeaderText("Where to load this file?");
...
//                try (ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream("saveFile.bin"))) {
//                    serializeTo = (toSerialize) inputStream.readObject();
//                } catch (FileNotFoundException e) {
...
//
//
//                itemsToDo = FXCollections.observableArrayList(serializeTo.castToDo);
//                listToDo.setItems(itemsToDo);
//
//                itemsIn = FXCollections.observableArrayList(serializeTo.castIn);
//                listIn.setItems(itemsIn);
//
//                itemsDone = FXCollections.observableArrayList(serializeTo.castDone);
//                listDone.setItems(itemsDone);


        });

//        menuExportFile.setOnAction((ActionEvent t) -> {
...
//
//            if (file != null) {
//                serializeTo.castToDo = new ArrayList<>(listToDo);
//                serializeTo.castIn = new ArrayList<>(listIn);
//                serializeTo.castDone = new ArrayList<>(listDone);
//
//                try(ObjectOutputStream objectOutputStream = new ObjectOutputStream(new FileOutputStream(file.getAbsolutePath()))) {
...
//WYJĄTKI DO ODCZYTU
    void fileNotFoundExInfo()
    {
        Alert infoAlert = new Alert(AlertType.INFORMATION);
        infoAlert.setTitle("Expection");
        infoAlert.setHeaderText("File not found Exception!");
        infoAlert.showAndWait();
    }
    ...
    void ioExInfo()
    {
        Alert infoAlert = new Alert(AlertType.INFORMATION);
        infoAlert.setTitle("Expection");
        infoAlert.setHeaderText("IO Exception!");
        infoAlert.showAndWait();
    }

    void classNotFoundExInfo()
    {
        Alert infoAlert = new Alert(AlertType.INFORMATION);
        infoAlert.setTitle("Expection");
        infoAlert.setHeaderText("ClassNotFound Exception!");
        infoAlert.showAndWait();
    ...
    void moveItem(ListView<Item> list1, ListView<Item> list2)
    {
