...
public class FulfillmentCenter implements Comparator<String>
{
    ...
    public double currentQuantity = 0;                              //aktualna ilość produktów w magazynie
    ...
    {
        if(listOfProducts.contains(product))            //jeśli lista zawiera produkt
        {
            listOfProducts.remove(product);             //usuwa obiekt
            product.quantity--;                         //zmniejsza ilość o 1
        }
    }
    ...
    public void removeProduct(Item product)
    {
        Iterator<Item> iterator = listOfProducts.iterator();    //przechodzi po kolejnych elementach listy
        try {
                if (compareToName(product) == 0)                     //porównuje nazwy, jeśli takie same to usuwa
                {
                    listOfProducts.remove(product);             //obsluga wyjątku nie ma produktu
                } else {
                    throw new IllegalArgumentException("Błąd, nie ma produktu");
                }
            //} System.out.println("Nie ma produktu"); {
            //throw new IllegalArgumentException("Błąd, nie ma produktu");
        } catch(InputMismatchException exception) { System.out.println("Bląd");}
    }
